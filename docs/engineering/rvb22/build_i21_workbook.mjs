...
const input=process.env.RVB22_WORKBOOK_INPUT || '/workspace/scratch/6686d1f90cd8/inputs/review_current/GR86_CCA_RevB_ERB_CCB_Review_Register.xlsx';
const outputDir=process.env.RVB22_OUTPUT_DIR || '/workspace/scratch/6686d1f90cd8/outputs/rvb22_i21';
const audit=W+'/control/workbook_verification_i21';
...
for(const sh of Object.values(sheets)){
 ...
 sh.getRange('A1:K2000').unmerge();
 sh.getRange('A1:K2000').clear({applyTo:'contents'});
 sh.getRange('A1:K2000').conditionalFormats.deleteAll();
}
...
title(sheets['RVB22 Criteria'],'G','RVB22 I21 current criteria','Each original criterion retains its own closure. Desktop/model work and remaining native, physical or supplier conditions are explicit.');
...
title(sheets['RVB22 Redlines'],'F','RVB22 I21 redline dispositions','Findings and their actual corrections remain visible; a bounded model is identified separately from a native or supplier acceptance gate.');
...
title(sheets['RVB22 Inputs'],'C','RVB22 I21 inputs and operating bounds','Values and model conditions are source inputs. Edit with traceability; changing an assumption can invalidate its dependent closure.');
...
title(sheets['RVB22 Gates'],'D','RVB22 I21 remaining gates','Gate actions identify the remaining qualification or native evidence. They do not erase completed source corrections or model results.');
...
title(sheets['RVB22 Evidence'],'C','RVB22 I21 evidence hashes','One source reference per criterion. SHA256 values are supplied by the controlled consolidation; an absent hash is shown explicitly.');
...
const dashboard=sheets['RVB22 Current'];title(dashboard,'H','GR86 CCA I21 engineering review','Current counts are calculated from the original review register. Criteria, redlines and physical qualification gates are separate totals.');
...
for(let i=0;i<originalSheets.length+newNames.length;i++){const name=[...originalSheets,...newNames][i];const range=name==='RVB22 Current'?'A1:H'+Math.max(23,yr-1):name==='Review Register'?'G4:N6':name==='RVB22 Criteria'?'A1:G8':name==='RVB22 Redlines'?'A1:F7':name==='RVB22 Inputs'?'A1:C8':name==='RVB22 Gates'?'A1:D8':name==='RVB22 Iterations'?'A1:D8':name==='RVB22 Evidence'?'A1:C8':'A1:H12';const blob=await wb.render({sheetName:name,range,scale:1,format:'png'});await fs.writeFile(audit+'/final_'+String(i+1).padStart(2,'0')+'.png',new Uint8Array(await blob.arrayBuffer()));}
