...
def model(step,cu_um,eps,chassis,ambient=65,Pscale=1,direct_R=None):
 ...
 direct=overlap(box(68.3,11.5,72,28))
 ...
 for it in range(100):
  ...
  M=base+diags(loss+gc);rhs=q+ambient*loss+65*gc;new=spsolve(M,rhs)
 ...
 return {'mesh_mm':step,'effective_continuous_Cu_um':cu_um,'epsilon':eps,'chassis_contacts':chassis,'direct_R_K_W':direct_R,'direct_contact_mean_C':float(np.sum(direct*T)/sum(direct)),'cells':N,'iterations':it+1,'max_temperature_C':float(max(T)),'C206_region_mean_C':cT,'C206_region_max_C':float(max(T[probe>0])),'C206_region_max_plus10K_C':float(max(T[probe>0])+10),'source_region_mean_C':temperatures,'heat_to_chassis_W':heat_out,'total_heat_W':sum(q),'max_energy_residual_W':float(max(abs(residual))),'temperature_iteration_change_K':float(diff),'probe_region_mm':[78.95,16.25,86.25,20.55]}
if __name__=='__main__':
 results=[]
 for cu in [48,96,160]:
  for R in [5,10,15]:
   for e in [.3,.6]:results.append(model(.5,cu,e,True,direct_R=R))
 r={'status':'EXPLORATORY_SOURCE_POSITION_SENSITIVITY_NOT_NATIVE_THERMAL_PROOF','contact_mm':[68.3,11.5,72,28],'cases':results}
 (D/'DIRECT_CONTACT_MODEL.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(results))
