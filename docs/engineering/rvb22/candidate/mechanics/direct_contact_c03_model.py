...
def model(step,cu_um,eps,chassis,ambient=65,Pscale=1,direct_R=None,landing_C=65):
 ...
 direct=overlap(box(66.6,11,72,28.8))
 if direct_R is not None:
  gc+=(1/direct_R)*direct/sum(direct);areas-=direct
 ...
 for it in range(100):
  film=(T+273.15+Ta)/2;k,nu,al,Pr,_=air(film,70000);Ra=9.80665/film*abs(T+273.15-Ta)*H**3/(nu*al);Nu=(.825+.387*np.maximum(Ra,1e-8)**(1/6)/(1+(.492/Pr)**(9/16))**(8/27))**2;hs=.7*Nu*k/H;Tk=T+273.15;hrad=eps*SIGMA*(Tk+Ta)*(Tk*Tk+Ta*Ta);loss=(hs+hrad)*areas
  M=base+diags(loss+gc);rhs=q+ambient*loss+landing_C*gc;new=spsolve(M,rhs)
  diff=max(abs(new-T));T=.5*T+.5*new
  if diff<1e-7:break
 residual=base@T+(loss+gc)*T-rhs
 probe=overlap(box(78.95,16.25,86.25,20.55));cT=float(np.sum(probe*T)/sum(probe));heat_out=float(sum(gc*(T-landing_C)))
 temperatures={name:float(np.sum(overlap(shape)*T)/sum(overlap(shape))) for name,P,shape in heat_groups}
 return {'mesh_mm':step,'effective_continuous_Cu_um':cu_um,'epsilon':eps,'chassis_contacts':chassis,'direct_R_K_W':direct_R,'landing_C':landing_C,'direct_contact_mean_C':float(np.sum(direct*T)/sum(direct)),'cells':N,'iterations':it+1,'max_temperature_C':float(max(T)),'C206_region_mean_C':cT,'C206_region_max_C':float(max(T[probe>0])),'C206_region_max_plus10K_C':float(max(T[probe>0])+10),'source_region_mean_C':temperatures,'heat_to_chassis_W':heat_out,'total_heat_W':sum(q),'max_energy_residual_W':float(max(abs(residual))),'temperature_iteration_change_K':float(diff),'probe_region_mm':[78.95,16.25,86.25,20.55]}
if __name__=='__main__':
 results=[]
 for cu in [48,96,120,160]:
  for landing in [65,70]:
   for heat in [4.5571,4.815]:
    results.append(model(.5,cu,.3,True,Pscale=heat/4.79,direct_R=8,landing_C=landing))
 r={'status':'EXPLORATORY_SOURCE_POSITION_SENSITIVITY_NOT_NATIVE_THERMAL_PROOF','contact_mm':[66.6,11,72,28.8],'total_contact_R_design_K_W':8,'cases':results}
 (D/'DIRECT_CONTACT_C03_MODEL.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(results))
