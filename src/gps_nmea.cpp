...
bool nmeaCoordToDegrees(const char *ddmm, const char *hemi, double &out) {
  ...
  int len = dot ? static_cast<int>(dot - ddmm) : static_cast<int>(std::strlen(ddmm));
...
int clampMillis(int value) {
  if (value < 0) return 0;
  if (value > 999) return 999;
  return value;
}

int clampSats(int value) {
  if (value < 0) return 0;
  if (value > 63) return 63;
  return value;
}

double clampHdop(double value) {
...
bool parseRmcSentence(char *line, RmcData &out) {
  ...
  if (!nmeaChecksumOk(line, len)) return false;

  out = RmcData{};
  const char *fields[kMaxFields];
  int nf = splitCsv(line, len, fields, kMaxFields);
  ...
  if (time && std::strlen(time) >= 6) {
    ...
    if (dot) {
      int scale = 100;
      for (const char *p = dot + 1; *p && std::isdigit(static_cast<unsigned char>(*p)) && scale > 0; ++p) {
        ms += (*p - '0') * scale;
        scale /= 10;
      }
    }
    out.millis = clampMillis(ms);
  }

  out.valid = (fields[2] && fields[2][0] == 'A');

  double coord = 0.0;
  if (nmeaCoordToDegrees(fields[3], fields[4], coord)) {
    out.has_latitude = true;
    out.latitude_deg = coord;
  }
  if (nmeaCoordToDegrees(fields[5], fields[6], coord)) {
...
bool parseGgaSentence(char *line, GgaData &out) {
  ...
  if (fields[7] && fields[7][0]) {
    out.has_sats = true;
    out.sats = clampSats(std::atoi(fields[7]));
  } else {
    out.has_sats = false;
    out.sats = 0;
  }
  ...
  if (fields[8] && fields[8][0]) {
    out.has_hdop = true;
    out.hdop = clampHdop(std::atof(fields[8]));
  } else {
    out.has_hdop = false;
    out.hdop = 0.0;
  }
